    ...
    stage('Push image') {
     ...
     stage('Provision Dev App') {
        ...
        withCredentials([string(credentialsId: 'morpheus-credentials', variable: 'bearer')]) {
            String morpheusUrl = 'https://sandbox3.lh.local/api/apps'
			...
			def response = httpRequest customHeaders: [[name: 'Authorization', value: "BEARER ${bearer}"]], url: 'https://sandbox3.lh.local/api/apps'
